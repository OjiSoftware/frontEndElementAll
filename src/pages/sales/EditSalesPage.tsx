...
export default function EditSalesPage() {
  ...
  const handleConfirmSubmit = async () => {
    try {
      ...
      // 1. Check if client exists and we have an ID to update. If yes, update it.
      if (clientId) {
        const clientPayload = {
          name: formData.name,
              ...
              <div className="p-4 border-t border-white/5 space-y-4 shadow-inner">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" name="postalCode" value={formData.postalCode} onChange={handleChange} className="w-full bg-slate-700/90 border border-gray-500 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-indigo-400 outline-none transition-all" placeholder="1000" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-300 mb-1">País</label>
                    <input type="text" name="country" value={formData.country} onChange={handleChange} className="w-full bg-slate-700/90 border border-gray-500 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-indigo-400 outline-none transition-all" placeholder="País" />
                  </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-4 border-t border-white/10 gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto">
              ...
              <button
                ...
                disabled={formData.details.length === 0 || isLoading}
        ...
        <ConfirmModal
          ...
          message={
            <div className="text-slate-300 text-sm">
              ¿Estás seguro de registrar estos cambios en la venta? El total será de
