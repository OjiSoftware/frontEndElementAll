...
export default function CartPage() {
    ...
    return (
        <div className="flex flex-col min-h-screen w-full bg-[#f1f3f5] border-b border-gray-200">
            ...
            <div className="w-full max-w-[1187px] mx-auto py-6 flex-grow px-0 sm:px-4 ">
                ...
                <div className="flex flex-col lg:flex-row gap-6 items-start">
                    {/* Lista de productos (Columna Principal - ~70%) */}
                    ...
                    {/* Resumen de orden (Columna Derecha - 30%) */}
                    <div className="flex-[0_0_100%] lg:flex-[1_1_30%] bg-white rounded-2xl max-sm:rounded-none shadow-sm border border-gray-200 max-sm:border-x-0 p-5 sticky top-8 flex flex-col gap-3 font-lato w-full">
                        <div className="flex flex-col gap-2 pb-3 border-b border-gray-100">
                            ...
                            <div className="flex justify-between items-center text-[13px] text-gray-500">
                                ...
                                <span>${(totalPrice - (totalPrice / 1.21)).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                            </div>
                        </div>

                        <div className="flex flex-col gap-1 mb-2 mt-1">
                            <div className="flex justify-between items-center">
                                <span className="font-bold text-gray-800 text-base">Total Final</span>
                                <span className="font-black text-xl text-[#16a34a]">${totalPrice.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="text-[10px] text-gray-400 text-right uppercase tracking-tight">
                                Precio sin impuestos nacionales: ${(totalPrice / 1.21).toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                            </div>
                        </div>

                        <button
                            className="w-full py-3 bg-[#16a34a] text-white text-[0.95rem] font-bold rounded-lg hover:bg-[#15803d] transition shadow-md flex items-center justify-center gap-2 mt-2"
                            onClick={() => alert("Redirigiendo a la pasarela de pago...")}
                        >
                            Ir a Pagar
                        </button>
                        <p className="text-[11px] text-gray-500 text-center leading-tight">
                            * Los recargos por financiación en cuotas con tarjeta de crédito corren por cuenta del cliente.
                        </p>
                        ...
                        {!showConfirmClear ? (
                            ...
                            <div className="flex flex-col gap-2 mt-2">
                                <span className="text-xs text-red-600 text-center font-bold">¿Seguro que deseas vaciar el carrito?</span>
