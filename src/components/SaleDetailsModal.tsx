    ...
    >
      ...
      >
        ...
        <div className="mt-4 md:mt-6 flex justify-end border-t border-slate-700 pt-4 gap-2 md:gap-3 shrink-0">
          <button
            className="px-4 md:px-6 py-3 md:py-2 font-bold rounded-lg text-sm flex-1 md:flex-none border border-slate-600 text-slate-300 bg-transparent transition-all duration-300 cursor-pointer hover:bg-slate-500/20 hover:text-white hover:border-slate-400"
            onClick={onClose}
          >
            Cerrar
          </button>
          <Link
            ...
            className={`px-4 md:px-6 py-3 md:py-2 font-bold rounded-lg transition text-sm flex items-center justify-center gap-2 flex-1 md:flex-none ${
              currentStatus === 'CANCELLED' || currentStatus === 'COMPLETED'
                ? 'bg-indigo-900/40 text-slate-400 cursor-not-allowed pointer-events-none shadow-none border border-indigo-500/10'
